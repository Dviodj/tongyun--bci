"""
BCICIV 2b - 最终优化版 (100 epochs预训练 + 目标被试微调)
"""
import sys, os
...
def train(X, y, n_epochs=100, batch=64, lr=0.001, wd=0.01):
...
def finetune(model, X, y, n_epochs=30, batch=32, lr=0.0001):
    dev = torch.device('cpu')
    # Freeze conv
...
def main():
    ...
    # Pretrain
    print("\n[2] 预训练EEGNet (100 epochs)...")
    model = train(Xa, ya, n_epochs=100, batch=64, lr=0.001)
    print("  完成")
    ...
    ft_scores = []
    for name, X, y in data:
        # Split own data for fine-tuning
        cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
        subj_scores = []
        for tr_idx, te_idx in cv.split(X, y):
            # Copy pretrained model
            m = train(X[tr_idx], y[tr_idx], n_epochs=30, batch=32, lr=0.0005)
            m = finetune(m, X[tr_idx], y[tr_idx], n_epochs=20)
            subj_scores.append(eval_model(m, X[te_idx], y[te_idx]))
        ft_scores.append(np.mean(subj_scores))
        print(f"  {name}: {ft_scores[-1]:.1%}")
    print(f"\n  FT LOSO Mean: {np.mean(ft_scores):.1%} +/- {np.std(ft_scores):.1%}")
    print("=" * 60)
